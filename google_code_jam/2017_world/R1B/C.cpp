...
int main() {
	...
	REP(ttt, test_cases) {
		...
		ll INF = 1LL<<60;
		VVI w(N, VI(N, INF));
		REP(i, N) REP(j, N) if(dist[i][j]!=-1) w[i][j] = dist[i][j];
		REP(k, N) REP(i, N) REP(j, N) w[i][j] = min(w[i][j], w[i][k]+w[k][j]);

		vector<vector<double>> ti(N, vector<double>(N, INF));
		REP(i, N) REP(j, N) if(w[i][j] <= En[i]) ti[i][j] = (double)w[i][j] / Sp[i];
		REP(k, N) REP(i, N) REP(j, N) ti[i][j] = min(ti[i][j], ti[i][k]+ti[k][j]);
...
//			double ans = 1e100;
//	//		DD(d.V);
//			REP(i, N) ans = min(ans, d.V[E[qi]*N+i]);

			printf("%.9f ", ti[S[qi]][E[qi]]);
		}
		printf("\n");
//		return 0;
	}
