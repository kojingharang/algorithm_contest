...
int main() {
	...
	REP(ttt, test_cases) {
		...
		REP(loop, 100) {
			double mid = (lo+hi)/2;
			if(A < cos(mid)/cos(M_PI/4)) lo=mid;
			else hi=mid;
//			DD(lo);
//			DD(hi);
		}
...
//		DD(lo);
		...
		int ans = 0;
		/*
		0.5 0 0 -> 0.5*cos(th) 0.5*sin(th) 0
		0 0.5 0 -> 0.5*cos(th) 0.5*sin(th) 0
		0 0 0.5
		*/
		cout<<"Case #"<<ttt+1<<":"<<endl;
		cout<<fixed<<setprecision(20);
		cout<<0.5*cos(th)<<" "<<0.5*sin(th)<<" "<<0<<endl;
		cout<<0.5*cos(M_PI/2+th)<<" "<<0.5*sin(M_PI/2+th)<<" "<<0<<endl;
		cout<<"0 0 0.5"<<endl;
//		return 0;
	}
	return 0;
