...
bool check(vector<Outfit>& Os, int J, int P, int S, int K, bool dup=false) {
//	DD("check");
//	REP(i, Os.size()) {
//		auto o = Os[i];
...
int main() {
	...
	//					DD("NG");
			...
			bool ok=false;
			int cnt=0;
			while(!check(Os, J, P, S, K, true)) {
				random_shuffle(ALL(Os));
				cnt++;
				if(cnt>1000) break;
			}
			if(check(Os, J, P, S, K, true)) break;
		}
