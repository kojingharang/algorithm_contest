...
int main() {
	...
	while(cin>>N) {
		VI A(N);
		REP(i, N) cin>>A[i];
		ll li=*max_element(ALL(A));
		vector<vector<PII>> h(2, vector<PII>(li+1));
		REP(i, 2) REP(v, li+1) h[i][v].SE=v;
		REP(i, N) {
			h[i%2][A[i]].FI++;
		}
//		DD(h);
		REP(i, 2) sort(ALLR(h[i]));
//		DD(h);
		ll ans=INF;
		REP(i, 2) {
			ll oi = h[i][0].SE==h[1-i][0].SE ? 1:0;
			minUpdate(ans, N-(h[i][0].FI+h[1-i][oi].FI));
		}
		cout<<ans<<endl;
//		break;
	}
