...
int main() {
	...
	while(cin>>N>>S) {
		...
		vector<vector<vector<modll>>> dp(N+1, vector<vector<modll>>(S+1, vector<modll>(3)));
		dp[0][0][0] = 1;
		REP(i, N) {
			REP(si, S+1) {
				dp[i+1][si][0] += dp[i][si][0];
				dp[i+1][si][1] += dp[i][si][0] + dp[i][si][1];
				dp[i+1][si][2] += dp[i][si][0] + dp[i][si][1] + dp[i][si][2];
				ll nsi = si+A[i];
				if(nsi<S+1) {
					dp[i+1][nsi][1] += dp[i][si][0] + dp[i][si][1];
					dp[i+1][nsi][2] += dp[i][si][0] + dp[i][si][1];
				}
			}
		}
//		DD(dp);
