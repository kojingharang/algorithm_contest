...
// dp[r][g][b] ... 1<=i<=max(r,g,b) まで塗って一番右の赤がr 緑がg 青がbだったときの組み合わせ総数
modll dp[302][302][302];
	...
	while(cin>>N>>M) {
		...
		REP(r, N+1) REP(g, N+1) REP(b, N+1) {
			...
			for(auto& p : co[m]) {
				ll L = p.first;
				ll X = p.second;
				if(X==1 && !(w[1]<L)) ok=false;
				if(X==2 && !(w[0]<L && L<=w[1])) ok=false;
				if(X==3 && !(L<=w[0])) ok=false;
			}
//			DD(r);DD(g);DD(b);DD(ok);
			if(ok) {
				dp[nm][g][b] += dp[r][g][b];
				dp[r][nm][b] += dp[r][g][b];
				dp[r][g][nm] += dp[r][g][b];
