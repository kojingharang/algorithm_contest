...
int main() {
	...
	while(cin>>N>>D>>A) {
		...
		ll ans = 0;
		SegTree<Add> st(N+1);
		REP(i, N) st.update(i, XH[i].SE - (0<=i-1 ? XH[i-1].SE : 0LL));
		REP(i, N) {
			ll r = XH[i].FI+D*2;
			ll ri = upper_bound(ALL(XH), MP(r, INF)) - XH.begin() - 1;
			ll co = (max(0LL, st.query(0, i+1))+A-1)/A;
			ans += co;
			st.update(i, st.query(i, i+1)-co*A);
			st.update(ri+1, st.query(ri+1, ri+2)+co*A);
		}
		cout<<ans<<endl;
	}
