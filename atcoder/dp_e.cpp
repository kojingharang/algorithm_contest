...
int main() {
	...
	while(cin>>D>>S) {
		ll N = S.size();
		// 0: S 未満未確定(Sと等しい) 1: S 未満確定
		vector<vector<modll>> dp(2, vector<modll>(D));
		dp[0][0] = 1;
//		DD(dp);
		REP(i, N) {
			vector<vector<modll>> ndp(2, vector<modll>(D));
			// 0 -> 0
			REP(d, D) ndp[0][(d+(S[i]-'0'))%D] += dp[0][d];
			// 0 -> 1
			REP(d, D) REP(di, (S[i]-'0')) ndp[1][(d+di)%D] += dp[0][d];
			// 1 -> 1
			REP(d, D) REP(di, 10) ndp[1][(d+di)%D] += dp[1][d];
			dp = ndp;
//			DD(dp);
		}
		modll ans = dp[1][0] + dp[0][0] - 1LL;
		cout<<ans<<endl;
//		break;
	}
