...
class YetAnotherCardGame {
	...
	int maxCards(vector <int> A, vector <int> B) {
		int N=A.size();
		vector<int> c; c.reserve(N*2);
		sort(ALL(A));
		sort(ALL(B));
		int k=0,j=0,p=0;
		while(j<N && B[j]<A[0]) if(p<B[j++]) ++k;
		p=0;
		int n=0;
		REP(i, N) {
			if(A[i]>p)
			{
				while(j<N && B[j]<=A[i]) {
					if(p<B[j]&&B[j]<A[i]&&(j<1||B[j]>B[j-1]))++k;
					++j;
				}
				if(j<N) {
					if(i>=N-1||A[i+1]>=B[j]) ++n,p=B[j++];
					else ++k;
				} else {
					++k;
					p=A[i];
		...
		return n*2+min(N-n,k);
	}
};

/*
	map<pair<int, pair<int, int>>, int> memo;
	int f(int turn, int v, int cur) {
		auto key = MP(turn, MP(v, cur));
		if(memo.count(key)) return memo[key];

		int putv = -1;
		int rv=0;
		REP(i, N) if(v<AB[turn][i]) {putv=AB[turn][i]; break;}
		if(putv==-1) return cur;
//		cout<<"f "<<turn<<" "<<v<<" "<<cur<<endl;
		// put
		rv = max(rv, f(1-turn, putv, 1+cur));
		// noput
		rv = max(rv, f(1-turn, putv, cur));
		memo[key] = rv;
		return rv;
	}
