...
class TheMountain {
	...
	int minSum(int H, int W, vector <int> IY, vector <int> IX, vector <int> IE) {
		...
		REP(ty, H) REP(tx, W) {
			int mi = -1;
			int ma = 0;
			REP(i, 4) if(ma < w[i][ty+1][tx+1]) { ma = w[i][ty+1][tx+1]; mi=i; }
			int DX[]={0, -1, 0, -1};
			int DY[]={0, 0, -1, -1};
			ll v0 = dp[0][ty+DY[mi] +1][tx+DX[mi] +1];
			ll v1 = dp[1][ty+DY[mi] +1][tx+DX[mi]+1 +1];
			ll v2 = dp[2][ty+DY[mi]+1 +1][tx+DX[mi] +1];
			ll v3 = dp[3][ty+DY[mi]+1 +1][tx+DX[mi]+1 +1];
			if(v0==-1 || v1==-1 || v2==-1 || v3==-1) continue;
			ll lans = v0+v1+v2+v3;
			cout<<tx<<" "<<ty<<" - "<<v0<<" "<<v1<<" "<<v2<<" "<<v3<<" = "<<lans<<endl;
			ans = min(ans, lans);
		}
