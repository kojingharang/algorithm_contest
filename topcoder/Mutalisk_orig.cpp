...
class Mutalisk {
	...
	vector<VVI> ex;
	int co;
	void f(int v, int idx, int n) {
		if(n==1) {
			w[idx]=v;
			REP(i, 3) m[i]=max(m[i], w[i]);
			ex[vv].PB(w);
			co++;
//			cout<<w<<endl;
			return;
		...
		RANGE(i, 1, N+1) {
			REP(a, A) REP(b, B) REP(c, C) {
				if(dp[i-1][a][b][c]) {
					REP(j, ex[x[i-1]].size()) {
						auto& X = ex[x[i-1]][j];
						if(a+X[0]<A && b+X[1]<B && c+X[2]<C) dp[i][a+X[0]][b+X[1]][c+X[2]]=1;
					}
				}
			}
			REP(a, A) REP(b, B) REP(c, C) {
				if(dp[i][a][b][c]) cout<<i<<" "<<a<<" "<<b<<" "<<c<<endl;
			}
		}
		ll ans = 1LL<<60;
		REP(a, A) REP(b, B) REP(c, C) {
			if(dp[N][a][b][c]) ans=min(ans, (ll)max(a, max(b, c)));
		}
		return ans;
	}
};
