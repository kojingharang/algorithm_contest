...
//class BiconnectedDiv1Ref {
//	public:
//	int minimize(vector <int> w1, vector <int> w2) {
//		ll N = w1.size()+1;
//		VVI best(N-2, VI(2, -INF));
//		best[0][1]=0;
//		RANGE(i, 1, N-2) {
//			best[i][0] = best[i-1][1]+w2[i];
//			best[i][1] = max(best[i-1][0], best[i-1][1] + w1[i]);
//		}
//		int res = accumulate(ALL(w1), 0) + accumulate(ALL(w2), 0);
//		res -= best[N-3][1];
//		return res;
//	}
//};

// dp[i][j][k][l] iまで見た時に, i-2の次数がj, i-1の次数がk, iの次数がl のときの最小の重み和
ll dp[111][5][5][5];
//vector<ll> used[111][5][5][5];
...
//		REP(i, N) REP(j, 5) REP(k, 5) REP(l, 5) used[i][j][k][l].clear();
		// 1-2おかない
		dp[2][2][1][1] = w1[0] + w2[0];
//		used[2][2][1][1].PB(w1[0]);
//		used[2][2][1][1].PB(w2[0]);
		// 1-2おく
		dp[2][2][2][2] = w1[0] + w2[0] + w1[1];
//		used[2][2][2][2].PB(w1[0]);
//		used[2][2][2][2].PB(w1[1]);
//		used[2][2][2][2].PB(w2[0]);
		RANGE(i, 3, N) REP(j, 5) REP(k, 5) REP(l, 5) if(dp[i-1][j][k][l] < INF) {
			// (i-1, i), (i-2, i) のうちどちらかは置かないといけない(connectedにならない)
			// (i-1, i) を置く
			{
				ll nl = min(5, l+1);
				if(j%2==0) {
					ll nv = dp[i-1][j][k][l] + w1[i-1];
					if(nv < dp[i][k][nl][1]) {
//						used[i][k][nl][1] = used[i-1][j][k][l];
//						used[i][k][nl][1].PB(w1[i-1]);
						dp[i][k][nl][1] = min(dp[i][k][nl][1], nv);
//						cout<<endl<<"w1"<<endl;
//						cout<<i<<" "<<k<<" "<<nl<<" "<<1<<" "<<dp[i][k][nl][1]
//							<<" <- "
//							<<i-1<<" "<<j<<" "<<k<<" "<<l<<" "<<dp[i-1][j][k][l]<<" + "<<w1[i-1]<<endl;
//						DD(used[i][k][nl][1]);
					}
				}
			}
			// (i-2, i) を置く
			{
				ll nk = min(5, k+1);
				if(j%2==0) {
					ll nv = dp[i-1][j][k][l] + w2[i-2];
					if(nv < dp[i][nk][l][1]) {
//						cout<<endl<<"w2"<<endl;
//						used[i][nk][l][1] = used[i-1][j][k][l];
//						used[i][nk][l][1].PB(w2[i-2]);
						dp[i][nk][l][1] = min(dp[i][nk][l][1], nv);
//						cout<<i<<" "<<nk<<" "<<l<<" "<<1<<" "<<dp[i][nk][l][1]
//							<<" <- "
//							<<i-1<<" "<<j<<" "<<k<<" "<<l<<" "<<dp[i-1][j][k][l]<<" + "<<w2[i-2]<<endl;
//						DD(used[i][nk][l][1]);
					}
				}
			}
			// (i-1, i), (i-2, i) を置く
			{
				...
				ll nk = min(5, k+1);
				if(j%2==0) {
					ll nv = dp[i-1][j][k][l] + w1[i-1] + w2[i-2];
					if(nv < dp[i][nk][nl][2]) {
//						cout<<endl<<"w1 w2"<<endl;
//						used[i][nk][nl][2] = used[i-1][j][k][l];
//						used[i][nk][nl][2].PB(w1[i-1]);
//						used[i][nk][nl][2].PB(w2[i-2]);
						dp[i][nk][nl][2] = min(dp[i][nk][nl][2], nv);
//						cout<<i<<" "<<nk<<" "<<nl<<" "<<2<<" "<<dp[i][nk][nl][2]
//							<<" <- "
//							<<i-1<<" "<<j<<" "<<k<<" "<<l<<" "<<dp[i-1][j][k][l]<<" + "<<w1[i-1]<<" "<<w2[i-2]<<endl;
//						DD(used[i][nk][nl][2]);
					}
				}
			}
...
//		RANGE(i, 2, N) {
//			REP(j, 5) REP(k, 5) REP(l, 5) cout<<i<<" "<<j<<" "<<k<<" "<<l<<" "<<dp[i][j][k][l]<<endl;
//		}
		RANGE(i, 2, N) {
			REP(j, 5) REP(k, 5) REP(l, 5) {
//				cout<<i<<" "<<j<<" "<<k<<" "<<l<<" "<<endl;
//				DD(used[i][j][k][l]);
//				cout<<"SUM "<<accumulate(ALL(used[i][j][k][l]), 0LL)<<endl;
			}
		}
//		DD(used[N-1][2][2][2]);
//		DD(dp[N-1][2][2][2]);
